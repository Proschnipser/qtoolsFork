...
def signalp6_parallel(args):
    ...
    if Path(fastafile).stat().st_size ==0:
        print("Fastafile empty", orflist[1],orflist[-4])
    else:
...
fna_files = list(Path(chondrichthyes).rglob("*_chunked.fna"))
COLS = [
...
for k1,v1 in dfdict.items(): #iterate over genomes
    ...
    for orf, orflist in orfdict.items():
        fna_files =  list(Path(chondrichthyes+"/"+orflist[0]+"/").rglob("*chunked.fna"))
        if(fna_files != []):
            ...
            #print(sequence)
            fastafile=signaldir+orflist[1]+orflist[-4]+".fasta"
...
csv_out="/data/joscha/Data/hmmer_results_chunked_TANGO1.csv"
print(csv_out)
print(hitsdf)

...
Ecdysozoa=(
    ...
    .apply(lambda g: g.sample(1))  # pick a random row per unique value
    .sample(n=min(len(set(Ecdysozoa["Sequence cutted"].values)),70), random_state=42)
)
print(len(Ecdysozoa))
...
print(len(Others))
df=pd.concat([euteleostomi_TANGO1, Ecdysozoa, Spiralia, Others])
print(len(df))
fasta_out=os.path.splitext(filepath)[0]+"_andHMMer"+"_names.fasta"
hmmer_df= hmmer_df[hmmer_df["query_name"].str.contains("TANGO1")]
seqrecords=[]

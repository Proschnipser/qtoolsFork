...
print(hmmer_df, [len(i) for i in hmmer_df["Sequence cutted"]])
...
sample_size=70
threshold_length=89
euteleostomi_TANGO1=df[df["OC"].str.contains("Euteleostomi") & (df["name"]=="TANGO1") & (df["Sequence cutted"].str.len()>=threshold_length)]
euteleostomi_TANGO1=(
...
Spiralia=df[df["OC"].str.contains("Spiralia") & (df["Sequence cutted"].str.len()>=threshold_length)]
Spiralia=(
    Spiralia.groupby("Sequence cutted", group_keys=False)
    .apply(lambda g: g.sample(1))  # pick a random row per unique value
    .sample(n=min(len(set(Spiralia["Sequence cutted"].values)),70), random_state=42)
)
print(len(Spiralia))
Others= df[~df["OC"].str.contains("Spiralia|Ecdysozoa|Euteleostomi") & (df["Sequence cutted"].str.len()>=threshold_length)]
Others=(
    Others.groupby("Sequence cutted", group_keys=False)
    .apply(lambda g: g.sample(1))  # pick a random row per unique value
    .sample(n=min(len(set(Others["Sequence cutted"].values)),70), random_state=42)
)
print(len(Others))
...
seqrecords= extract_seqrecords(hmmer_df, threshold_length, seqrecords)
